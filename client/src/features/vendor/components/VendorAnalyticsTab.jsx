    ...
    <div className="space-y-8">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <Panel key={i} tone={s.tone} className="p-8">
            <p className="text-xs font-black uppercase tracking-widest text-muted">{s.label}</p>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <Panel tone="dark" className="lg:col-span-2 p-8">
          <h3 className="text-xl font-black text-heading mb-8 border-b border-white/10 pb-4">Last 7 Days Trend</h3>
          {trend.length === 0 ? (
            <div className="flex h-72 items-center justify-center text-muted font-bold text-lg border-2 border-dashed border-white/10 rounded-2xl">No sales data yet</div>
          ) : (
              ...
              {trend.map(entry => {
                ...
                return (
                  <div key={entry.date} className="group relative flex flex-1 flex-col items-center justify-end h-full">
                    <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-raised rounded-xl p-3 text-center shadow-[0_10px_20px_rgba(0,0,0,0.5)] border border-white/10 z-10 pointer-events-none min-w-[80px]">
                       <p className="text-sm font-black text-amber-500">{entry.orders} orders</p>
                       <p className="text-[11px] font-bold text-heading mt-1">{formatCurrency(entry.revenue)}</p>
                    </div>
                    <div 
                    ...
                    />
                    <p className="mt-4 text-xs font-black text-muted uppercase tracking-widest">{entry.label}</p>
                  </div>
                );
              })}
            </div>
        ...
        <Panel tone="dark" className="p-8">
          <h3 className="text-xl font-black text-heading mb-8 border-b border-white/10 pb-4">Menu Coverage</h3>
          <div className="flex h-64 items-center justify-center">
            <div className="relative h-56 w-56 rounded-full border-[12px] border-white/5 flex items-center justify-center shadow-[inset_0_0_30px_rgba(0,0,0,0.5)] bg-card">
               <div className="text-center z-10">
                 <p className="text-5xl font-black text-amber-500 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]">{stats.activeMenuItems || 0}</p>
                 <p className="text-xs font-black text-muted mt-2 uppercase tracking-widest">Active Items</p>
               </div>
          ...
          <p className="text-center text-sm font-black text-muted mt-6 bg-white/5 p-3 rounded-xl border border-white/5">Out of <span className="text-heading">{stats.totalMenuItems || 0}</span> total created items</p>
        </Panel>
      </div>
    </div>
  );
